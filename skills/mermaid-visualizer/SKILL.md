...
### Rule 6: No HTML Tags for `content` params in `create_note` mcp function of `triliumnext-mcp`

For example, use  `\n` over `<br>` tags 
✅ RIGHT: Node[Line 1\nLine 2] → Use standard newline character if needed
❌ WRONG: Node[Line 1<br/>Line 2] → Triggers "HTML content detected" validation error
...
* **Important**: The `content` argument passed to the tool must be the **mermaid code**. Do not wrap it in Mermaid code blocks (like `mermaid ... `) inside the function call, unless specifically requested to store it as a code snippet. It should be the direct JSON data.

### 3. Calling the MCP Tool

You must use the `create_note` tool from `triliumnext-mcp`: https://github.com/tan-yong-sheng/triliumnext-mcp. If the `triliumnext-mcp` is not installed, just directly output the mermaid syntax.
